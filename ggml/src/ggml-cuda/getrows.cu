...
template<typename dst_t>
static __global__ void k_get_rows_mxfp6_e2m3_bw(
        const void * __restrict__ src0, const int32_t * __restrict__ src1, dst_t * __restrict__ dst,
    ...
    const tensor_mxfp6 * __restrict__ tensor = (const tensor_mxfp6 *) src0;
    const int64_t frags_per_row = ggml_cuda_mxfp6_e2m3_frags_per_row(ne00);
    const int64_t tile_rows = (ne01 + MXFP6_TILE_ROWS - 1) / MXFP6_TILE_ROWS;
    const int64_t frags_per_plane = tile_rows * frags_per_row;
    const size_t row_major_plane_bytes = nb01 * (size_t) ne01;
    ...
    for (int64_t z = blockIdx.z; z < ne11*ne12; z += gridDim.z) {
        for (int64_t i00 = blockIdx.y*blockDim.x + threadIdx.x; i00 < ne00; i00 += gridDim.y*blockDim.x) {
            ...
            const int row = src1[i10*s10 + i11*s11 + i12*s12];

            const size_t plane_bytes = (size_t) i11 * nb02 + (size_t) i12 * nb03;
            const int64_t plane = row_major_plane_bytes == 0 ? 0 : (int64_t) (plane_bytes / row_major_plane_bytes);
            const tile_mxfp6_frag * __restrict__ frags =
                (const tile_mxfp6_frag *) ((const char *) tensor + MXFP6_HEADER_OFFSET) + plane * frags_per_plane;

            const int64_t frag_col = i00 / QK_MXFP6_E2M3;
            const int rem = i00 - frag_col * QK_MXFP6_E2M3;
            const int word = rem >> 2;
            const int lane_byte = rem & 3;
            const int row_in_tile = row & 15;
            const tile_mxfp6_frag & frag = frags[(row / 16) * frags_per_row + frag_col];

            const uint32_t q4 = ggml_cuda_mxfp6_e2m3_frag_codes_to_lanes(frag, row_in_tile, word);
            const uint8_t code = (q4 >> (8 * lane_byte)) & 0x3F;
            const float tensor_scale = tensor->weight_scale;
            const uint8_t scale_code = frag.scale[((row_in_tile & 7) * 4) + (row_in_tile >> 3)];
            const float scale = tensor_scale * ggml_cuda_e8m0_to_fp32(scale_code) *
                (1.0f / 8.0f);

            dst_t * dst_row = dst + i10*s1 + i11*s2 + i12*s3;
            dst_row[i00] = ggml_cuda_cast<dst_t>(scale * float(kvalues_mxfp6_e2m3[code]));
        }
    }
}
...
template<typename dst_t>
static void get_rows_cuda_mxfp6_e2m3_bw(
        const void * src0_d, const int32_t * src1_d, dst_t * dst_d,
    ...
    k_get_rows_mxfp6_e2m3_bw<<<block_nums, block_dims, 0, stream>>>(
        src0_d, src1_d, dst_d, ne00, ne01, ne11, ne12, s1, s2, s3, nb01, nb02, nb03, s10, s11, s12);
}
...
static void ggml_cuda_get_rows_switch_src0_type(
    ...
    switch (src0_type) {
        ...
        case GGML_TYPE_MXFP6_E2M3:
            ...
            get_rows_cuda_mxfp6_e2m3_bw(src0_d, src1_d, dst_d,
