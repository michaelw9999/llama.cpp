...
static inline __host__ __device__ uint32_t ggml_cuda_nvfp4_tile_q_word(
...
static inline __host__ __device__ uint32_t ggml_cuda_nvfp4_tile_scale_word(
...
static inline uint32_t ggml_cuda_nvfp4_pack8(const uint8_t * p, int shift) {
    return
        ...
        for (int64_t block_col = 0; block_col < dst_blocks_per_row; ++block_col) {
            ...
            for (int row_in_tile = 0; row_in_tile < rows_in_tile; ++row_in_tile) {
                ...
                for (int frag = 0; frag < frags_in_block; ++frag) {
                    ...
                    tile.regs[lane_base + 0][row_half + 0] = ggml_cuda_nvfp4_pack8(p0, 0);
                    tile.regs[lane_base + 1][row_half + 0] = ggml_cuda_nvfp4_pack8(p0, 4);
                    tile.regs[lane_base + 2][row_half + 0] = ggml_cuda_nvfp4_pack8(p1, 0);
                    tile.regs[lane_base + 3][row_half + 0] = ggml_cuda_nvfp4_pack8(p1, 4);
                    tile.regs[lane_base + 0][row_half + 2] = ggml_cuda_nvfp4_pack8(p2, 0);
                    tile.regs[lane_base + 1][row_half + 2] = ggml_cuda_nvfp4_pack8(p2, 4);
                    tile.regs[lane_base + 2][row_half + 2] = ggml_cuda_nvfp4_pack8(p3, 0);
                    tile.regs[lane_base + 3][row_half + 2] = ggml_cuda_nvfp4_pack8(p3, 4);
